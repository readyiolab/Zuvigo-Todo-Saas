...
function InputOTPSlot({
  ...
  return (
    <div
      ...
      className={cn(
        "relative flex size-10 items-center justify-center border border-transparent border-b-input bg-transparent text-body transition-[color,border-color] outline-none first:rounded-md last:rounded-md aria-invalid:border-b-destructive data-[active=true]:z-10 data-[active=true]:border-b-ring dark:aria-invalid:border-b-destructive/50",
        className,
      )}
      {...props}
    >
      ...
      {hasFakeCaret && (
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          <div className="h-4 w-px animate-caret-blink bg-foreground duration-(--duration-fast)0" />
        </div>
      )}
    </div>
  );
